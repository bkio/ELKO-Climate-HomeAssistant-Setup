...
#ELKO
DEFAULT_ELKO_MIN_TEMPERATURE = 4
DEFAULT_ELKO_MAX_TEMPERATURE = 30
DEFAULT_ELKO_BATHROOM_MIN_TEMPERATURE = 1
DEFAULT_ELKO_BATHROOM_MAX_TEMPERATURE = 10
ELKO_HVAC_MODE_ON_OFF_DP = 116
...
def flow_schema(dps):
    ...
    return {
        ...
        vol.Optional(CONF_IS_ELKO): bool, #ELKO
        vol.Optional(CONF_IS_ELKO_BATHROOM): bool #ELKO
    }
...
class LocaltuyaClimate(LocalTuyaEntity, ClimateEntity):
    ...
    async def async_set_hvac_mode(self, hvac_mode):
        ...
        if self._config.get(CONF_IS_ELKO) is True:
            if hvac_mode == HVACMode.OFF:
                ...
                await self._device.set_dp(False, ELKO_HVAC_MODE_ON_OFF_DP)
            return    
    ...
    def status_updated(self):
        ...
        if self.has_config(CONF_TARGET_TEMPERATURE_DP):
            self._target_temperature = (
                self.dps_conf(CONF_TARGET_TEMPERATURE_DP) * self._target_precision * elko_factor #ELKO
            )

        if self.has_config(CONF_CURRENT_TEMPERATURE_DP):
            self._current_temperature = (
                self.dps_conf(CONF_CURRENT_TEMPERATURE_DP) * self._precision * elko_factor #ELKO
            )
